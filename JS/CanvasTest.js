...
let DrawTools = {
    ...
    Utar: (ev) => {
        ...
        } else {SpeechAndBluetooth
            return {
                x: (ev.targetTouches[0].clientX - d.x),
                y: (ev.targetTouches[0].clientY - d.y)
            }
        }
    },
    Save: (ev) => {
...
let CameraTool = {
    ...
    init: (ev) => {
        document.querySelector("#camera").addEventListener("click", CameraTool.shutter);
    },
    shutter: async (ev) => {
